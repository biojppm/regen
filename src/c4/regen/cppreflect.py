...
class Annotation(CodeEntity):
    ...
    def contains(self, name):
        if not name in self.names: return False
        return (getattr(self, p).val != None)
...
class AnnotationValue:
    ...
    def __init__(self, n, v=None):
        self.name = n
        self.val = v if (v != None) else True

    def __str__(self):
        s = ""
        s += self.name
        if self.val != None and self.val != True:
            s += "=" + str(self.val)
        return s
...
def run(writer, enum_generator=None, class_generators=None, in_args=None, clang_version=None):
    ...
    #
    clu.load_clang(opts.clang_libdir, opts.clang_version, opts.clang_version_fallback)
    #
    source_files = []
    for a in args:
        sf = SourceFile(a)
